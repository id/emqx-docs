...
认证缓存会保存来自外部资源的认证请求结果。其典型工作流程如下：

1. 客户端发起连接并触发认证。
2. EMQX 检查缓存中是否已有匹配结果：
   - 如果找到有效结果，则记为 **缓存命中**，无需查询外部后端；
   - 如果未找到结果，则记为 **缓存未命中**，EMQX 会查询外部认证后端。
3. 从外部后端返回的认证结果将被存入缓存，用于后续请求，并计入 **缓存写入** 指标。

该机制有助于降低认证延迟、减少外部资源调用，并在高负载下维持系统响应能力。

### 启用并配置认证缓存

您可以通过 EMQX Dashboard 启用并配置认证缓存：

1. 进入 Dashboard，导航至 **访问控制** -> **认证** 页面；
2. 点击页面右上角的 **外部资源缓存设置** 按钮，右侧会弹出配置面板；
3. 在侧边面板中，使用 **启用外部资源缓存** 开关开启或关闭缓存功能。启用后，可配置以下缓存参数：
   - **最大缓存条目数**：设置每个节点可缓存的最大认证结果条数。默认值为 `1,000,000`。
   - **最大内存使用量**：限制缓存可使用的最大内存。默认值为 `100 MB`。
   - **缓存 TTL**：指定缓存结果的有效期。默认值为 `1 分钟`。
4. 点击 **Update** 应用设置。

所有配置将在整个集群范围内生效，确保各节点行为一致。

### 查看认证缓存状态

您可以在 Dashboard 中查看认证缓存的使用情况：

1. 点击 **外部资源缓存设置** 旁边的下拉箭头，选择 **外部资源缓存状态**；
2. 右侧将弹出一个侧边面板，显示缓存指标；
3. 在面板右上角的下拉菜单中，可选择查看 **集群级** 或 **单节点级** 的统计数据。

指标包括：

- **内存使用量**：缓存当前占用的内存总量；
- **缓存条目数**：当前缓存中存储的认证结果总数；
- **缓存命中**：EMQX 从缓存中成功命中结果的次数，避免了对外部后端的访问；
  - 展示数据包括：**当前速率**、**5 分钟平均值**、**历史最大值**
- **缓存未命中**：缓存中未命中，触发外部后端查询的次数；
  - 展示数据包括：**当前速率**、**5 分钟平均值**、**历史最大值**
- **缓存写入**：未命中后新增写入缓存的认证结果次数；
  - 展示数据包括：**当前速率**、**5 分钟平均值**、**历史最大值**

在面板底部，会显示各节点的 **内存使用量**、**缓存条目数** 和 **缓存命中数** 的统计信息。

您可以使用右上角的 **刷新** 和 **重置** 按钮来更新或清除统计数据。
