...
| bytes_burst | 最大消息发布流量突发速率（单客户端） | 单个客户端在常规 `数据发布速率` 基础上允许突发发送的数据量 |  |
| messages_rate   | 最大消息发布速率（单客户端） | 单个客户端每秒发布的消息条数             | 暂停接收客户端消息     |
| messages_burst | 最大消息发布突发速率（单客户端） | 在常规 `消息发布速率` 基础上，每个客户端允许突发发送的消息数量 |  |
