...
| 配置项                 | 类型    | 可取值          | 默认值        | 说明                                                                              |
| -----------------------| ------- | --------------- | ------------------------------------------------------------------------------------------------- |
| mqtt.max_inflight      | 整数 | (0, 65536)      | 32            | 飞行窗口的长度限制, 0 表示没有限制                                   |
| mqtt.max_mqueue_len    | 整数 | [0, ∞)          | 1000          | 消息队列长度限制，0 表示没有限制                                        |
| mqtt.mqueue_store_qos0 | 枚举    | `true`, `false` | true          | 客户端离线时，EMQX 是否将 QoS 0 消息存储到消息队列中 |
