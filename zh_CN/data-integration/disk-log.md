...
EMQX 提供内置的系统日志功能，用于记录错误、警告等运行时事件，而 Disk Log 集成则面向不同的目标：它用于将 MQTT 消息数据和 客户端事件实际写入磁盘，以实现长期存储和离线处理。
...
- **轻量独立部署**：无需依赖外部数据库或云存储，特别适合边缘计算、离线运行或存储受限环境。
...
9. 如需高级配置，可展开[**高级设置**](#高级设置)选项。

10. 保持其他配置默认，点击**创建**完成 Sink 配置。页面将返回规则配置，新的 Sink 会自动添加为动作。

11. 最后点击**创建**，完成整条规则的创建。
