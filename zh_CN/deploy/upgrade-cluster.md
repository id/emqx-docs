...
3. 迁移 EMQX 集群。
   ...
   EMQX 提供了一个[迁移工具](https://github.com/emqx/emqx-data-converter/releases)可以帮助将现有的 EMQX 4.4 集群的配置迁移到新的 EMQX `@CE_VERSION@` 集群中。该工具能够自动化迁移过程，确保平稳过渡。
...
5. 将生产环境切换到升级后的集群。更新您的 DNS 记录、负载均衡器或任何其他相关配置，以将流量定向到新的集群。在切换后密切监控系统，确保运行平稳。迁移后，请务必检查生成的配置，确保它们符合预期。
