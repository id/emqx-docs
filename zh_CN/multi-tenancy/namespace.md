...
2. **通过 EMQX 提取 `client_attrs.tns` 自动创建命名空间**
   当客户端连接时，EMQX 可以根据客户端的 `client_attrs.tns` 属性自动创建命名空间。这种方式通常用于无需手动创建命名空间的场景，适用于大规模自动化的部署。

   **使用场景**：适用于客户端动态连接并且需要自动为每个租户或客户端创建独立命名空间的情况。
...
在 Dashboard 左侧菜单中点击**管理** ->**命名空间**。在**命名空间**页面中您可以查看、创建、编辑和删除命名空间，还可以管理连接到命名空间的客户端。
...
如果您想要编辑某个特定命名空间的设置，可以在命名空间列表中，点击该命名空间**操作**列中的**编辑**。
...
通过该端点，您可以设置速率限制、会话限制和其他命名空间特定的配置。有关示例配置，请参见[快速入门](#快速入门为命名空间配置速率限制器)部分。
